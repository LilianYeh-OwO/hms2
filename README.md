...
The base image, Gaia, contains commonly used APT packages (see <https://gitlab.com/DYSK_Labs/data-team/gaia/-/blob/master/docker/base.gpu.Dockerfile?ref_type=heads>) and a pre-installed PyTorch.
Two versions of PyTorch, `torchv1` and `torchv2`, are provided.
Edit the `Dockerfile` and `pyproject.toml` to choose your preference.

To prevent re-installation across different projects, re-using the packages already installed in the base image can improve both build time and space efficiency.
Do not modify the versions of `torch` and `torchvision` specified in `pyproject.toml`, unless you have reasons.

### Add APT package dependency

If your project uses additional APT package (e.g., libtiff-dev), edit `Dockerfile`.

### Manage versioned data

We use [Data Version Control (DVC)](https://gitlab.com/DYSK_Labs/data-team/ml-team-guideline/-/blob/main/data_version_management/DVC.md) and [Data Vault](https://gitlab.com/DYSK_Labs/data-team/ml-team-guideline/-/blob/main/data_version_management/data-vault.md) to manage and store versioned data, respectively, including training annotations, model weights, and test artifacts.

To pull these data, set up the credentials and call `dvc pull`.

If it is the first time you would like to push versioned data to Data Vault, create a bucket, set up the remote URL and credentials, call `dvc add <data>`, and call `dvc push`.

See the guides for more information:
- DVC: <https://gitlab.com/DYSK_Labs/data-team/ml-team-guideline/-/blob/main/data_version_management/DVC.md>
- Data Vault: <https://gitlab.com/DYSK_Labs/data-team/ml-team-guideline/-/blob/main/data_version_management/data-vault.md>

### Update the template version

[ai-project-template](https://gitlab.com/DYSK_Labs/data-team/ai-projects/ai-project-template) is constantly updated.

To make your AI project conform to the latest template (or of a centain version):
```bash
git remote add template git@gitlab.com:DYSK_Labs/data-team/ai-projects/ai-project-template.git
git fetch template
git checkout -b bump-template-[template version]
git merge --allow-unrelated-histories template-[template version]  # You will probably have to resolve the merge conflict
git push --set-upstream origin bump-template-[template version]
```

Then, resolve the merge conflict. For `poetry.lock`, just re-lock after `pyproject.toml` is handled by:
```bash
...
git push --set-upstream origin bump-template-[template version]
```
, and create a merge request to merge `bump-template-[template version]` back to `main` on Gitlab.
